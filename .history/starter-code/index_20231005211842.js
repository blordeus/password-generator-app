...
function generate(length) {
  ...
  const function generatePassword(length) {
    ...
    if (
      ...
      let password = "";
      const randomValuesArray = new Uint32Array(length);
      window.crypto.getRandomValues(randomValuesArray);
  
      for (let i = 0; i < passwordLength; i++) {
        password += chars[randomValuesArray[i] % chars.length];
      }
  ...
  }symbols = document.getElementById("inclSymbol");
